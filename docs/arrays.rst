...
plain python numbers, which :code:`torch.stack` does not accept.
Those are passed through as a list instead, which is what kingdon does by default anyway.
...
.. code-block::
    ...
    >>> alg = Algebra.fromname("3DPGA", wrapper=torch.compile, values_asarray=asarray)

3. Write vectorized code whenever possible. :code:`kingdon` multivectors are not themselves array types, but they enable the same programming paradigm.
...
.. code-block::
    ...
    >>> O = alg.point([0, 0, 0])         # Point to consider; in this case the origin
