...
Opting in to the :code:`torch` backend sets a :code:`values_asarray` of its own, and does a little
more besides; see :doc:`backends/torch`.
...
.. code-block::
    ...
    >>> alg = Algebra.fromname("3DPGA", wrapper=torch.compile, backend="torch")
...
And a multivector over :code:`torch` coefficients can be handed to a :code:`torch.nn` module
directly, einops and all. That has a page of its own: :doc:`backends/torch`.
