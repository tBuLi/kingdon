...
1. Preallocate the result array and tell kingdon to use it:
...
2. Write a wrapper function that ensures that all code generated by kingdon will return a (new) numpy array:
...
.. code-block::
    ...
    >>>         if len(res) == 0:
    >>>             return np.empty(0)
    >>>         return np.stack(np.broadcast_arrays(*res))
    >>>     return wrapper
    >>> alg = Algebra(3, 0, 1, wrapper=asarray)
