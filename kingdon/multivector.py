...
class MultiVector(metaclass=MultiVectorType):
    ...
    def frommatrix(cls, algebra, matrix):
        ...
        """
        return cls(algebra=algebra, values=matrix[..., 0])

    @classmethod
    def sanitize_keys_grades(cls, algebra, keys=None, grades=None, full_layout=None) -> tuple[str, ...]:
        """
    ...
    def fromname(cls, algebra, name: str, keys=None, grades=None, symbolcls=None, full_layout=None):
        ...
        values = list(symbolcls(f'{name}{blade[1:]}') for blade in keys)
    ...
    @property
    def type_layout(self) -> dict[str, float | EllipsisType]:
        r"""
        Layout of :code:`type(self)`: a mapping from blade-string key to either
    ...
    def __str__(self):
        ...
        canon_sorted_vals = {print_key(key): val for key, val in keysvals}
        str_repr = ' + '.join(
    ...
    def __setattr__(self, basis_blade, value):
        ...
        if basis_blade in self.keys():
            self._values[self.keys().index(basis_blade)] = value
        else:
...
def _union_keys(mvs: list[MultiVector]) -> tuple[str, ...]:
    """
